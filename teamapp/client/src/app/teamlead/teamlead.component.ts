...
export class TeamleadComponent implements OnInit {
  ...
  openEditListPanel(listid: string, listname: string): void {
    this.isEditListPanel = true;
    this.worklistid = listid;
    this.addeditlistname = listname;
  }
  ...
  removeMember(membername: string) {
    this.work.removemember(new RemovePeople(this.projectid, membername)).subscribe((data) => {
        this.servermessage = data.message;
        if(this.servermessage == "removed") {
          this.loadProjectInf();
        }
