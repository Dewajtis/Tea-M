                        ...
                        if(count2>0) {
                            ...
                            user.save()
                                .then(item => {
                                    ...
                                    responce.send({message: "saved",id: user._id});
                                });
                        }
                    });
            }
...
app.get("/login", jsonParser, function(request,responce) {
    const userUsername = request.query.username;
    ...
    User.count({username: userUsername, password: userPassword})
        .then(count => {
            if(count==0) {
                ...
                User.find({username: userUsername},{password:0})
                    .then(cursor => {
                        responce.send({message: "login", id: cursor[0]._id});
                    });
            }
        })
        .catch(err => {
